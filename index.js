...
function getFlavorByIndex(arr, place){
    return console.log(arr[place]);
}
...
copy(originalFlavors, 'poppingFlavors');
...
function filterByWord(arr, flavor, newArrName){
    newArrName = [];
    for(let i = 0; i < arr.length; i++){
        if(arr[i].includes(flavor)){
           newArrName.push(arr[i]); 
        }
    }
    return newArrName;
}

console.log(filterByWord(originalFlavors, 'Chocolate', 'holidayFavorites'));
